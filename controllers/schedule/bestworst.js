...
module.exports = {
    post: async (req, res) => {
		try {
            ...
            await scheduleModel.findOne({ _id: req.params.scheduleid, "bestWorst.userInfo": decoded.userId }, async (err, schedule) => {
                if(!schedule) {
                    let user = { userInfo: decoded.userId, best: req.body.best, worst: req.body.worst }
                    ...
                    await scheduleModel.updateOne({"bestWorst.userInfo": decoded.userId}, {$set: {"bestWorst.$.best": req.body.best, "bestWorst.$.worst": req.body.worst}}, async (err, result) => {
                        if (err) return res.status(400).json({message: "best worst 업데이트 실패" });
                        await scheduleModel.findOne({ _id: req.params.scheduleid, "bestWorst.userInfo": decoded.userId }, async (err, data) => {
                            console.log(data)
