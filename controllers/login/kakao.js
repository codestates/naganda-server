...
module.exports = {
  post: async (req, res) => {
    ...
    axios({
      ...
      .then(async (res2) => {
        const alreadyUser = await usersModel.findOne({ email: res2.data.kakao_account.email });
        if(alreadyUser){
          ...
          await newUser.save();
          const findUser = await usersModel.findOne({ email: res2.data.kakao_account.email });
